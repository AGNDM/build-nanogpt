#!/bin/bash
#SBATCH --job-name=nanogpt-smaller
#SBATCH --account=a131
...
#SBATCH --output=log_smaller/%x-%j.out           
#SBATCH --error=log_smaller/%x-%j.err            
...
srun --uenv=pytorch/v2.8.0:v1 --view=default bash -c " \
    ...
    python -m torch.distributed.run --standalone --nproc_per_node=4 train_gpt2_smaller.py \
"
