#!/bin/bash
#SBATCH --job-name=nanogpt-linear-lr
#SBATCH --account=a131
...
#SBATCH --output=log_linear_lr/%x-%j.out           
#SBATCH --error=log_linear_lr/%x-%j.err            
...
srun --uenv=pytorch/v2.8.0:v1 --view=default bash -c " \
    ...
    python -m torch.distributed.run --standalone --nproc_per_node=4 train_gpt2_linear_lr.py \
"
